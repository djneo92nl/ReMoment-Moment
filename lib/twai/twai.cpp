...
esp_err_t can_init(twai_timing_config_t t_config, twai_filter_config_t f_config, twai_general_config_t g_config)
{
    ...
    {
        Serial.println("Driver installed"); // Log driver installation success
    }
    else
    {
        Serial.println("Failed to install driver"); // Log driver installation failure
        return ESP_FAIL;
    }
    ...
    {
        Serial.println("Driver started"); // Log driver start success
    }
    else
    {
        Serial.println("Failed to start driver"); // Log driver start failure
        return ESP_FAIL;
    }
    ...
    {
        Serial.println("CAN Alerts reconfigured"); // Log alert reconfiguration success
    }
    else
    {
        Serial.println("Failed to reconfigure alerts"); // Log alert reconfiguration failure
        return ESP_FAIL;
    }
...
uint32_t can_read_alerts()
{
    ...
    {
        Serial.println("Alert: TWAI controller is in error passive state.");
    ...
    if (alerts_triggered & TWAI_ALERT_BUS_ERROR)
    {
        ...
        printf("Bus error count: %" PRIu32, twaistatus.bus_error_count);
    ...
    if (alerts_triggered & TWAI_ALERT_TX_FAILED)
    {
        ...
        printf("TX buffered: %ld" PRIu32, twaistatus.msgs_to_tx);
        return TWAI_ALERT_TX_FAILED;
    }

    if (alerts_triggered & TWAI_ALERT_TX_SUCCESS)
    {
        Serial.println("Alert: Transmission successful.");
        return TWAI_ALERT_TX_SUCCESS;
    }

    if (alerts_triggered & TWAI_ALERT_RX_QUEUE_FULL)
    {
        Serial.println("Alert: RX queue full, frame lost.");
...
void can_write_Byte(twai_message_t message)
{
    ...
    {
        Serial.println("Message queued for transmission"); // Log success
    }
    else
    {
        Serial.println("Failed to queue message for transmission"); // Log failure
    }
}
